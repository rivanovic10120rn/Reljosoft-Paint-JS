...
function floodFill(position, color) {
    let tolerance = 100; //pokusati sve izmedju 5 i 20

    //funkcija unutar funkcije jer pucam vise
    function matchesColor(idx) {
        let dr = pixels.data[idx]     - originalColor.r;
        let dg = pixels.data[idx + 1] - originalColor.g;
        let db = pixels.data[idx + 2] - originalColor.b;
        let da = pixels.data[idx + 3] - originalColor.a;
    ...
    let originalColor = {
        ...
        a: pixels.data[linearCoordinates + 3] = 255 //samo settuj na 255 ako treba
    };
    ...
    color[3] = 255; //necu vise ni da ga pitam samo bre forceujemo alpha!
    ...
    //provera preko delte
    let dr = originalColor.r - color[0];
    let dg = originalColor.g - color[1];
    let db = originalColor.b - color[2];
    let da = originalColor.a - color[3];
    if ((dr*dr + dg*dg + db*db + da*da) <= tolerance*tolerance) {
        console.log(`First pixel matches (with tolerance), aborting floodFill`);
        return;
    }
    ...
    while (pixelStack.length > 0) {
        ...
        while (y-- >= 0 && matchesColor(linearCoordinates))
        {
        ...
        while (y++ < canvas.height && matchesColor(linearCoordinates))
        {
            pixels.data[linearCoordinates] = color[0];
            pixels.data[linearCoordinates + 1] = color[1];
            pixels.data[linearCoordinates + 2] = color[2];
            pixels.data[linearCoordinates + 3] = color[3];

            if (x > 0) {
                let leftBorder = linearCoordinates - 4;
                if (matchesColor(leftBorder)) {
                    if (!reachedLeft) {
            ...
            if (x < canvas.width - 1) {
                let rightBorder = linearCoordinates + 4;
                if (matchesColor(rightBorder)) {
                    if (!reachedRight) {
                        // if(isInPixelStack(x + 1, y, pixelStack)) {
                        //     pixelStack.push({x: x + 1, y: y});
                        //     reachedRight = true;
...
// popravka za hendlovanje antialiasinga - kaze internet da je threshold najbolje resenje za to
// const TOLERANCE = 8; // adjust
// const TOLERANCE_SQUARED = TOLERANCE * TOLERANCE;
//
// function matchesColor(i) {
//     let dr = pixels.data[i]   - originalColor.r;
//     let dg = pixels.data[i+1] - originalColor.g;
//     let db = pixels.data[i+2] - originalColor.b;
//     return (dr*dr + dg*dg + db*db) <= TOLERANCE_SQUARED;
// }
//samo cu ubaciti funkciju unutar funkcije jer me ubija ovo prebacivanje parametara
